...
class UR5ePushAndReachXYZEnv(MultitaskEnv, UR5eXYZEnv):
    def __init__(
            ...
            fix_goal=False,
            fixed_goal=(0, 0, 0.84, 0, 0),  # table top surface height .82, puck half height .02
            goal_low=(-.2, -.2, .84, -.2, -.2),
    ...
    def _set_goal_marker(self, goal):
        ...
        """
        self.data.site_xpos[self.model.site_name2id('hand-goal-site')] = (goal[:3])
        self.data.site_xpos[self.model.site_name2id('puck-goal-site')][:2] = (goal[3:])
...
class UR5ePushAndReachXYEnv(UR5ePushAndReachXYZEnv):
    def __init__(self, *args, hand_z_position=0.9, **kwargs):
        self.quick_init(locals())
        UR5ePushAndReachXYZEnv.__init__(self, *args, **kwargs)
